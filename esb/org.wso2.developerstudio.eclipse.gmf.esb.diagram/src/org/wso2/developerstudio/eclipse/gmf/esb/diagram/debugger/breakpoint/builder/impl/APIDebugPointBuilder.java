...
import org.wso2.developerstudio.eclipse.gmf.esb.diagram.debugger.messages.command.ESBAPIBean;
import org.wso2.developerstudio.eclipse.gmf.esb.diagram.debugger.messages.command.ESBAPIDebugPointMessage;
import org.wso2.developerstudio.eclipse.gmf.esb.diagram.debugger.messages.command.ESBAPIResourceBean;
import org.wso2.developerstudio.eclipse.gmf.esb.diagram.debugger.messages.command.ESBAPISequenceBean;
	...
	public ESBDebugPoint getESBDebugPoint(EsbServer esbServer,
		...
		if (proxyContainer instanceof ProxyServiceSequenceAndEndpointContainerEditPart) {
			if (part.reversed) {
				...
				sequenceType = API_OUTSEQ;
			} else {
				position = getMediatorPosition(
						apiResource.getOutputConnector(), selection);
				sequenceType = API_INSEQ;
			}
		} else {
		...
		ESBAPIResourceBean resourse = new ESBAPIResourceBean(
				ESBDebugerUtil.getMethodValuesFromResource(apiResource),
				apiResource.getUrlMapping(), apiResource.getUriTemplate());

		ESBAPIBean apiBean = new ESBAPIBean(api.getApiName(), resourse,
				sequenceType, new ESBMediatorPosition(position));
		ESBAPIDebugPointMessage apiDebugPoint = new ESBAPIDebugPointMessage(
				null, commandArgument, SEQUENCE, new ESBAPISequenceBean(apiBean));
		...
		if (proxyContainer instanceof ProxyServiceSequenceAndEndpointContainerEditPart) {
			if (abstractMediator.reversed) {
				...
				List<ESBDebugPoint> breakpontList = getDebugPointsRelatedToModification(
						resource, position, API_OUTSEQ, action);
				...
				List<ESBDebugPoint> breakpontList = getDebugPointsRelatedToModification(
						resource, position, API_INSEQ, action);
				if (MEDIATOR_INSERT_ACTION.equalsIgnoreCase(action)) {
					increaseBreakpointPosition(breakpontList);
				} else {
					decreaseBreakpointPosition(breakpontList);
